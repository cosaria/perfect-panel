...
type SystemLogEntry = Record<string, unknown> & {
  timestamp?: string;
};
        ...
        <ScrollArea className="bg-muted/30 h-[60vh] max-h-[80vh] min-h-[400px] w-full rounded-lg border p-1">
          {isLoading ? (
            ...
            <Accordion type="single" collapsible className="w-full">
              {logs?.map((log) => {
                const itemKey = `${String(log.timestamp || "unknown")}-${Object.keys(log).join("-")}`;

                return (
                  <AccordionItem key={itemKey} value={itemKey} className="px-4">
