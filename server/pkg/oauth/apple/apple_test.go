...
func handleAppleCallBack(ctx context.Context, request CallbackRequest) {
	...
	teamID := os.Getenv("PPANEL_TEST_APPLE_TEAM_ID")
	clientID := os.Getenv("PPANEL_TEST_APPLE_CLIENT_ID")
	keyID := os.Getenv("PPANEL_TEST_APPLE_KEY_ID")
	clientSecret := os.Getenv("PPANEL_TEST_APPLE_CLIENT_SECRET")
	if teamID == "" || clientID == "" || keyID == "" || clientSecret == "" {
		fmt.Println("apple test credentials are not configured")
		return
	}
	// validate the token
