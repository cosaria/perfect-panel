...
func TestUpdateTosConfigUpdatesFieldAndEvictsTosCache(t *testing.T) {
	updated := map[string]string{}
	var deletedKeys []string
	req := &types.TosConfig{
		TosContent: "<p>Terms updated</p>",
	}
	deps := Deps{
	...
	logic := NewUpdateTosConfigLogic(context.Background(), deps)

	err := logic.UpdateTosConfig(req)

	require.NoError(t, err)
	require.Equal(t, map[string]string{"TosContent": req.TosContent}, updated)
	require.ElementsMatch(t, []string{config.TosConfigKey}, deletedKeys)
}

func TestUpdateTosConfigReturnsDatabaseUpdateErrorWhenTransactionFails(t *testing.T) {
	deleteCalled := false
	deps := Deps{
		SystemModel: fakeSystemModel{
	...
	err := logic.UpdateTosConfig(&types.TosConfig{})

	requireSystemCodeError(t, err, xerr.DatabaseUpdateError)
	require.False(t, deleteCalled)
}

func TestUpdateTosConfigReturnsDatabaseUpdateErrorWhenCacheEvictionFails(t *testing.T) {
	deps := Deps{
		SystemModel: fakeSystemModel{
			transactionFn: func(_ context.Context, fn func(*gorm.DB) error) error {
				return fn(nil)
