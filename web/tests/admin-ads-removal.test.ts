...
describe("admin ads removal", () => {
	test("removes ads page files and device show_ads toggle", () => {
		expect(
			existsSync(
				new URL(
					"../apps/admin/src/pages/dashboard/ads/page.tsx",
					import.meta.url,
				),
			),
		).toBe(false);
		expect(
			existsSync(
				new URL(
					"../apps/admin/src/pages/dashboard/ads/ads-form.tsx",
					import.meta.url,
				),
			),
		).toBe(false);
		expect(
			existsSync(
				new URL("../apps/admin/locales/en-US/ads.json", import.meta.url),
			),
		).toBe(false);
		expect(
			existsSync(
				new URL("../apps/admin/locales/zh-CN/ads.json", import.meta.url),
			),
		).toBe(false);
