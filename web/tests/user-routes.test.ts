...
describe("user routes", () => {
	test("covers the unit 2 public and user navigation routes", async () => {
		...
		const { routes } = (await import("../apps/user/src/routes")) as {
			routes: RouteObject[];
		};
	...
	test("renders an explicit 404 page for unknown public routes", async () => {
		...
		const shellRoute =
			routes[0]?.children?.find((route) => route.path === undefined) ?? null;
		const publicNotFoundRoute =
			shellRoute?.children?.find((route) => route.path === "*") ?? null;
	...
	test("renders an explicit 404 page for unknown dashboard routes", async () => {
		...
		const dashboardNotFoundRoute =
			dashboardShellRoute?.children?.find((route) => route.path === "*") ??
			null;
