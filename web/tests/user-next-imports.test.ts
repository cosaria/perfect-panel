...
describe("user next import cleanup", () => {
	...
	test("user app directory has been retired in favor of src/pages", () => {
		expect(existsSync(new URL("../apps/user/app", import.meta.url))).toBe(
			false,
		);
		expect(existsSync(new URL("../apps/user/src/pages", import.meta.url))).toBe(
			true,
		);
	});
	...
	test("user source no longer depends on app compat wrappers", () => {
		expect(
			existsSync(new URL("../apps/user/src/compat", import.meta.url)),
		).toBe(false);
	});
});
