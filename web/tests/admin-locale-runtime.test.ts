...
const ADMIN_LOCALE_CHANGE_EVENT = "ppanel:admin-locale-change";
...
describe("admin locale runtime", () => {
  ...
  test("setLocale updates the cookie-backed locale and publishes a runtime change event", async () => {
    const { getClientLocale } = await import("../apps/admin/locales/client");
    const { setLocale } = await import("../apps/admin/utils/common");
    const receivedLocales: string[] = [];

    window.addEventListener(ADMIN_LOCALE_CHANGE_EVENT, (event) => {
