...
describe("admin next import cleanup", () => {
  ...
  test("admin app directory has been retired in favor of src/pages", () => {
    expect(existsSync(new URL("../apps/admin/app", import.meta.url))).toBe(false);
    expect(existsSync(new URL("../apps/admin/src/pages", import.meta.url))).toBe(true);
  });

  test("admin source no longer keeps a src/compat directory", () => {
    expect(existsSync(new URL("../apps/admin/src/compat", import.meta.url))).toBe(false);
  });
});
