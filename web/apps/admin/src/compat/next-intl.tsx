...
export const ADMIN_LOCALE_CHANGE_EVENT = "ppanel:admin-locale-change";
...
export function dispatchAdminLocaleChange(locale: string) {
  if (typeof window === "undefined") {
    return;
  }

  window.dispatchEvent(
    new CustomEvent<LocaleChangeDetail>(ADMIN_LOCALE_CHANGE_EVENT, {
      detail: { locale },
    }),
  );
}
...
export function NextIntlClientProvider({
  ...
  useEffect(() => {
    ...
    window.addEventListener(ADMIN_LOCALE_CHANGE_EVENT, handleLocaleChange);

    return () => {
      window.removeEventListener(ADMIN_LOCALE_CHANGE_EVENT, handleLocaleChange);
    };
  }, []);
