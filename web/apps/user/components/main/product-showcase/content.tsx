            ...
            <Card className="flex flex-col overflow-hidden rounded-lg shadow-lg transition-shadow duration-300 hover:shadow-2xl">
              ...
              <CardContent className="flex flex-grow flex-col gap-4 p-6 text-sm">
                <ul className="flex flex-grow flex-col gap-3">
                  {(() => {
                    let parsedDescription: SubscriptionDescription = {
                      description: "",
                      features: [],
                    };
                    try {
                      parsedDescription = JSON.parse(item.description) as SubscriptionDescription;
                    } catch {
                    ...
                    return (
                      <>
                        ...
                        {features?.map(
                          (feature: { type: string; icon: string; label: ReactNode }) => (
                            <li
                              className={cn("flex items-center gap-2", {
                                "text-muted-foreground line-through":
                                  feature.type === "destructive",
                              })}
                              key={`${feature.type}-${String(feature.label)}-${feature.icon}`}
                            >
                              {feature.icon && (
                                <Icon
                                  icon={feature.icon}
